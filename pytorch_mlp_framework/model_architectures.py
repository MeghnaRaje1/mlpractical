...
class ConvolutionalProcessingBlock_BatchNorm_RC(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlock_BatchNorm_RC, self).__init__()
...
class ConvolutionalDimensionalityReductionBlock_BatchNorm_RC(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvolutionalDimensionalityReductionBlock_BatchNorm_RC, self).__init__()
    ...
    def build_module(self):
        ...
        out = F.avg_pool2d(out, self.reduction_factor)
        x_residual = F.avg_pool2d(x, self.reduction_factor)
        ...
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=out.shape[1])
        out = self.layer_dict['bn_1'].forward(out)
        out = F.leaky_relu(out + x_residual)
